...
export default new Vuex.Store({
  state: {
    ...
    PERMALINKS : 'wp-json',
    APIBLOGPOSTS : '/wp/v2/posts?categories=5&_fields=id,title,date,content.rendered,excerpt.rendered,_links.wp:featuredmedia.0,_links.replies&_embed=replies, wp:featuredmedia',
    APIMAINPAGEPOSTS : '/wp/v2/posts?categories=4&_fields=title,content.rendered,acf.color,acf.link,_links.wp:featuredmedia.0&_embed=wp:featuredmedia', // фильтр запрашиваемых значений (заголовок, дата, контент,категория, теги, изображения) categories=4 --для главной
    posts : [],
    pages : [],
    loading : true,
...
try {
  return processedResponse[indexPost].excerpt.rendered.match(/(.+)(\[&hellip;])/)[1]
} catch (e){
...
async getUserName({commit}) {
  if (localStorage.getItem('token')){
    const checkUserName = await fetch('http://chub96u7.beget.tech/wp-json/wp/v2/users/me', {
      method: 'GET',
      headers: {
